...
describe('When a movie is disassembled', () => {
    ...
    it('Should be in the meta collection', async () => {
        ...
        expect(movie.magnets[0]).toContain(stream.infoHash)
        for (i of stream.sources){
            expect(movie.magnets[0]).toContain(i.replace(hostnamePattern, '$1'))
        }
        expect(stream.metaId).toEqual(movie.meta.id)
        expect(stream.title).toEqual(movie.meta.name)
    })
})