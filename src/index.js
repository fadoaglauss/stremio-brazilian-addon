...
addon.defineStreamHandler( async (args) => {
    if (args.id.startsWith('br')) return { streams: streamDao.getByMetaId(args.id) }
    else return { streams: [] }
})

addon.defineCatalogHandler( async(args) => {
    if (args.extra.search) {
        return { metas: metaDao.getAll() }
    } else if (args.type == 'movie') {
        return { metas: metaDao.getByCatalogId(args.id) }
    } else {
        throw new Error('Invalid Catalog Request')
    }
})

serveHTTP(addon.getInterface(), { port: PORT, getRouter });